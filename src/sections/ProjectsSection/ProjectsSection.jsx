...
const ActivityTag = memo(({ lastUpdated }) => {
    ...
    return (
        <div className="item-status-meta">
            <div className="status-group" suppressHydrationWarning>
                {tag && <div className="active-tag">{tag}</div>}
            </div>
            <div className="last-updated-text" suppressHydrationWarning>Updated: {lastUpdated}</div>
        </div>
    );
});
        ...
        <div id={name} className={`project-list-item ${speciality === "COMING SOON" ? "list-item-building" : ""} ${isExpanded ? 'active' : ''}`}>
            <div className="list-item-top" onClick={onToggle} style={{ cursor: 'pointer' }}>
                <div className="list-item-top-content">
                    <div className="item-title-group" suppressHydrationWarning>
                        <ProjectPill speciality={speciality} />
                        <h3 className="item-name">{name || github?.repo_name}</h3>
                    </div>
                    <div className="list-item-links" suppressHydrationWarning>
                ...
                <div className="project-read-more" suppressHydrationWarning>
...
const EmblaCarousel = memo(({ projects }) => {
    ...
    const onScroll = useCallback((emblaApi) => {
        ...
        emblaApi.scrollSnapList().forEach((scrollSnap, index) => {
            ...
            const inner = slide.querySelector('.embla__slide__inner');
            if (inner) {
                let scale = 1;
                let opacity = 1;

                if (isMobile) {
                    opacity = index === emblaApi.selectedScrollSnap() ? 0.7 : 1;
                } else {
