...
export const trackBlogView = async (blogId) => {
  try {
    ...
    const storageKey = process.env.NEXT_PUBLIC_BLOG_VIEWED_IDS_KEY;
    if (!storageKey) {
      throw new Error('NEXT_PUBLIC_BLOG_VIEWED_IDS_KEY environment variable is not set');
    }
...
export const isBlogLiked = (blogId) => {
  try {
    const storageKey = process.env.NEXT_PUBLIC_BLOG_LIKED_IDS_KEY;
    if (!storageKey) {
      return false;
    }
...
export const toggleBlogLike = async (blogId) => {
  try {
    ...
    const storageKey = process.env.NEXT_PUBLIC_BLOG_LIKED_IDS_KEY;
    if (!storageKey) {
      throw new Error('NEXT_PUBLIC_BLOG_LIKED_IDS_KEY environment variable is not set');
    }
